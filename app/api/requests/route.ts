...
export async function POST(request: NextRequest) {
  ...
  if (tickets_requested > remaining) {
    return NextResponse.json({ error: "チケットが不足しています。残り: " + remaining + "枚" }, { status: 400 });
  }

  const { data, error } = await supabase
    .from("requests")
    .insert({ title, tickets_requested, preferred_datetime: preferred_datetime || null, parent_comment: parent_comment || null, parent_name: parent_name || null, status: "pending" })
    .select()
    .single();

...
export async function PUT(
  ...
  const { action, child_comment, tickets_negotiated } = body;
  ...
  if (action === "approve") {
    ...
    updateData.child_comment = child_comment ?? null;
    ...
    updateData.tickets_negotiated = tickets_negotiated;
    updateData.child_comment = child_comment ?? null;
  } else if (action === "agree") {
    if (req.status !== "negotiating") return NextResponse.json({ error: "交渉中のリクエストのみ同意できます" }, { status: 400 });
    updateData.status = "approved";
    updateData.tickets_negotiated = req.tickets_negotiated;
  } else if (action === "withdraw") {
    if (!["pending", "negotiating"].includes(req.status)) {
      return NextResponse.json({ error: "取り下げできないリクエストです" }, { status: 400 });
    }

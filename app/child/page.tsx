      ...
      <div className="px-5 pt-4">
        ...
        {tab === "settings" && (
          <div className="bg-white border border-gray-200 p-5">
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">新しい上限枚数</label>
              <div className="flex gap-3">
                <input type="number" value={newLimit} onChange={e => setNewLimit(e.target.value)} min={1} className="flex-1 border border-gray-300 px-4 py-3 text-sm focus:outline-none focus:ring-1 focus:ring-[#1a1f3a] focus:border-[#1a1f3a]" />
                <button onClick={handleLimit} disabled={limitLoading || !newLimit} className="bg-[#1a1f3a] hover:bg-[#252b4a] disabled:opacity-40 text-white font-semibold px-5 py-3 text-sm transition-colors">{limitLoading ? "更新中..." : "更新"}</button>
              </div>
              {limitMsg && <p className="mt-3 text-sm text-gray-600">{limitMsg}</p>}
            </div>
            <div className="mt-6 bg-[#f8f7f4] border border-gray-200 p-4 text-sm text-gray-500">
